...
function ArtworkView() {
  ...
  const checkBookmark = async () => {
    if (userInfo.favorites && artwork._id) {
      const isInBookmark = userInfo.favorites.some(
        (e) => e._id === artwork._id.toString()
      );
  ...
  const checkFollowing = async () => {
    if (userInfo.following) {
      const isInFollowing = userInfo.following.some(
        (e) => e.username === artwork.uploader
      );
  ...
  useEffect(() => {
    if (id) {
      fetch(`${urlBackend}/artworks/${id}`)
        ...
        .catch((error) => {
          console.error(
            "Erreur lors de la récupération de l'œuvre d'art:",
            error
          );
        });
    }
    ...
    if (isBookmarked) {
      ...
      try {
        const response = await fetch(
          `${urlBackend}/users/bookmark/${username}/${artwork._id}`,
          {
            method: "DELETE",
            headers: {
  ...
  const handleFollowingClick = async () => {
    if (isFollowed) {
      ...
      try {
        const response = await fetch(
          `${urlBackend}/users/following/${username}/${artwork.uploader}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
  ...
  const getClassName = (title) => {
    if (!title) {
      return "titleArt-default"; // Classe par défaut si le titre est undefined
    }
    if (title.length < 58) {
      return "titleArt";
    } else {
      return "titleArtLittle";
    }
  };
        ...
        <div className={styles.container}>
          <div className={styles.artworkZone}>
            <div className={styles.titleZone}>
              <div className={getClassName(artwork.title)}>{artwork.title}</div>
              {username !== null ? (
                !isBookmarked ? (
                  <BookmarkBorderIcon
            ...
            <div className={`${styles.tagsTitle} scrollbar1`}>{listTags}</div>
          </div>
          <div className={styles.textZone}>
            <div className={styles.zoneArtist}>
              <div
                onClick={() => handleUsernameClick(artwork.uploader)}
                className={`titleArtworkTextZone ${styles.artist}`}
              >
                {artwork.uploader}
              </div>
              <div>
            ...
            <div className={`${styles.descriptionText} scrollbar1`}>
              {artwork.description}
            </div>
