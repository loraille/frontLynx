      ...
      switch (category) {
        case "artworks":
          response = await fetch(
            `${urlBackend}/searchs/artworks?title=${query}`
          );
          break;
        case "tags":
        ...
        case "artists":
          response = await fetch(
            `${urlBackend}/searchs/artists?username=${query}`
          );
          break;
        default:
          return;
  ...
  return (
    <div>
      <main className={styles.main}>
        ...
        <div>
          {category === "artworks" && (
            <h1 className="titlePage">Results for Artworks</h1>
          )}
          {category === "tags" && (
            <h1 className="titlePage">Results for Tags</h1>
          )}
          {category === "artists" && (
            <h1 className="titlePage">Results for Artists</h1>
          )}
        </div>
        <div>
          {total === 0 ? (
            <span className={styles.reponseText}>
              "No result found in <span className={styles.lynx}>Lynx</span>"
            </span>
          ) : (
            <span className={styles.reponseText}>
              "{total} {result} found"
        ...
        <div className={styles.cardsDisplay}>{cardsResult}</div>
      </main>
    </div>
  );
}
