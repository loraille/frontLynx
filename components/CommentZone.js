...
function CommentZone({ artwork, onClickSend }) {
  ...
  const commentsList = comments.map((comment, index) => {
    ...
    return (
      <div
        key={index}
        className={
          username === uploader ? styles.commentUploader : styles.comment
        }
      >
        <div
  ...
  const handleSendComment = async () => {
    ...
    try {
      const response = await fetch(
        `${urlBackend}/artworks/comment/${artwork._id}`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        if (commentAreaRef.current) {
          commentAreaRef.current.scrollTop =
            commentAreaRef.current.scrollHeight;
        }
      } else {
  ...
  return (
    <div>
      <div className={`${styles.commentArea} scrollbar1`} ref={commentAreaRef}>
        {commentsList}
      </div>
      <div className={styles.newComment}>
        <textarea
          ...
          disabled={
            !visitor || newComment.length >= 255
          } /* limit comment size to 255 : moved from send button */
        />
        ...
        <button
          ...
          onClick={
            !visitor ? onClickSend : handleSendComment
          } /* was: disabled={!visitor} */
        >
          Send
          <SendIcon />
        </button>
